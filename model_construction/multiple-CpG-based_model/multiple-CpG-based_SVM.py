...
#   Out_single=pd.DataFrame()
   for train_index, test_index in kf.split(X_find_ch,y1):
       ...
       param_grid = [{'C': [1e-3,1e-2,0.1,1.0,10,1e2,1e3],
                      'gamma':[1e-3,1e-2,1e-1,1.0,10,1e2,1e3]}]
    
       clf =SVR(kernel='rbf')
       
       kfold = KFold(n_splits=3, shuffle=False)
       
       grid = GridSearchCV(clf, param_grid, cv=kfold, scoring='neg_mean_absolute_error',n_jobs=-1) #r2
       
       grid.fit(X_sel_self, train_y)
      
       a=grid.predict(test_X_new)  
...
Out_pre.T.to_csv("SVM-multiple_target.txt",sep='\t')    
